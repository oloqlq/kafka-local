...
# 3. 요청/응답 데이터의 구조를 정의 + 유효성검사 틀을 제공하는 클레스구성 -> pydantic 사용
#    BaseModel를 상속받고-> pydantic 사용 가능함 -> 틀/구조 정의
#    클레스 구성원등중 클레스 맴버 -> 키값 활용 -> 타입 부여 -> 유효성 검사를 위해
...
# @app => 데커레이터 => 함수 안에 함수 가 존재하는 2중 구조 => 튻정함수에 공통 기능 부여시 유용
...
def predict( users:List[ReqData] ):# 요청 데이터 형태를 규정 -> 유효성 검사 자동 수행
    ...
    for user in users:
        ...
        '''
        # 2. 고객 1명단 신용평가 수행 (AI x, 간다한 가상 공식 적용) => 차후 실제 모델로 교체
